...
namespace EldritchArcana
{
    namespace NewMechanics
    {
        ...
        [ComponentName("Increase caster level by value")]
        [AllowedOn(typeof(Kingmaker.Blueprints.Facts.BlueprintUnitFact))]
        public class ArcaneEmpowermentLevel : RuleInitiatorLogicComponent<RuleCastSpell>, IInitiatorRulebookHandler<RuleCalculateAbilityParams>, IRulebookHandler<RuleCalculateAbilityParams>
        {
            ...
            public string display_name = "Arcane Empowerment";
            public BlueprintAbilityResource resource;
        ...
        [ComponentName("Increase caster DC by value")]
        [AllowedOn(typeof(Kingmaker.Blueprints.Facts.BlueprintUnitFact))]
        public class ArcaneEmpowermentDC : RuleInitiatorLogicComponent<RuleCastSpell>, IInitiatorRulebookHandler<RuleCalculateAbilityParams>, IRulebookHandler<RuleCalculateAbilityParams>
        {
            public BlueprintBuff buff;
            ...
            public SavingThrowType save_type;
            public int dc_base = 10;
            public string display_name = "Arcane Empowerment DC";
            public BlueprintAbilityResource resource;
            private int caster_DC_increase = -1;
            ...
            public void OnEventAboutToTrigger(RuleCalculateAbilityParams evt)
            {
                caster_DC_increase = -1;
                if (evt.Spell == null || evt.Spellbook == null || evt.Spell.Type != AbilityType.Spell)
                {
                    return;
                }
                caster_DC_increase = dice_value.Calculate(this.Fact.MaybeContext);
                evt.AddBonusDC(caster_DC_increase);
            }
            ...
            public override void OnEventDidTrigger(RuleCastSpell evt)
            {
                if (caster_DC_increase == -1)
                {
                    return;
                }
                Common.AddBattleLogMessage($"{Owner.CharacterName}: {display_name} increases caster DC by {caster_DC_increase}");
                if (resource != null)
                {
                    this.Owner.Resources.Spend((BlueprintScriptableObject)this.resource, 1);
                }
